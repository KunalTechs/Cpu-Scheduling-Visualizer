    ...
    <div className="w-full p-6 lg:p-10 space-y-8 bg-zinc-950/50 rounded-[2rem] border border-zinc-800/50 shadow-2xl">
      ...
      <div className="relative h-32 w-full bg-black rounded-3xl border-2 border-zinc-900 flex overflow-hidden group/chart shadow-[0_0_50px_-12px_rgba(59,130,246,0.2)]">
        {data.map((block, index) => {
          ...
          // IDLE is dark zinc, Processes get dynamic colors
          ...
          return (
            <motion.div
              ...
              style={{ 
                ...
                // Adding a CSS stripe pattern for IDLE blocks
                backgroundImage: isIdle 
                  ? `repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(255,255,255,0.05) 10px, rgba(255,255,255,0.05) 20px)` 
                  : 'none'
              }}
            >
              {/* Glowing Top Bar (Only for actual processes) */}
              {!isIdle && (
                <div
              ...
              <span 
                className={`font-black ${isIdle ? "text-zinc-600 text-sm" : "text-lg"}`} 
                style={{ color: isIdle ? "" : blockColor }}
              >
                {isIdle ? "IDLE" : block.id}
              </span>
              
              <span className="text-[8px] font-bold text-zinc-500 mt-1">
                {duration}ms
              </span>

              {/* Enhanced Tooltip */}
              <div className="absolute -top-16 opacity-0 group-hover:opacity-100 transition-all duration-300 z-50 pointer-events-none translate-y-2 group-hover:translate-y-0">
